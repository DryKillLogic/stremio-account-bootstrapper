...
import type { Platform } from '../api/platformApi';
import type {
  DebridEntry,
  AddonConfigContext,
  SquirrellyRenderer,
...
import { translateCollections } from '../utils/collections';
import { replaceAddonKey } from '../utils/addonConfig';

declare const Sqrl: SquirrellyRenderer;

export interface BuildPresetServiceParams {
  preset: string;
  language: string;
  extras: string[];
  customAddons: string[];
...
export async function buildPresetService(params: BuildPresetServiceParams) {
  ...
  const no4k = options.includes('no4k');
  const cached = options.includes('cached');
  const kids = options.includes('kids');
  const limit = preset === 'minimal' ? 5 : 10;
  const size = maxSize ? maxSize : '';
  ...
  // Remove addons on Nuvio
  if (platform === 'nuvio') {
    presetKeys = presetKeys.filter(
      (key: string) => key !== 'cinemeta' && key !== 'streailer'
    );
  }
